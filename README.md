...
- **缓存命中率**：`cached_tokens / prompt_tokens`。不传 system 时后端会注入它的 harness 提示词，那部分通常大量命中缓存，命中率会很高；传了 system 则提示词小、命中率低。侧栏左下角把它画成**汽车速度表式的圆盘仪表**：240° 刻度盘（每 6° 一根刻度线、每 30° 一根主刻度，标注 0/25/50/75/100），指针与彩色弧线同步指向当前值，中心是数字读数。按阈值着色：**低于 80% 红、80%~90% 橙、90% 及以上绿**；「指标」页的实时活动表与按模型表里每一行的缓存率用同一套阈值着色，所以可以直接横向比较不同模型的命中情况。
...
- **单次请求的两条横条**：同一张表里，每条请求各一行 —— 「缓存」列是该**这条请求自己**的命中率（同阈值着色 + 一条横条），「首字」列下方那条横条以**当前列表里最慢的一条为满格**。后者是**相对刻度**：用来看同一屏里谁比谁慢，不代表绝对快慢（这个工作负载的平均首字约 2.9 秒，绝对值看数字）。
...
| `panel.test.mjs` | 面板内嵌脚本可解析，且脚本里 `$('id')` 引用的元素在 HTML 里都存在；缓存仪表的刻度数与弧长几何自洽、所有坐标不越出 viewBox |
